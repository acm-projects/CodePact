...
app.get('/leaderboard', async (req, res) => {
  try {
   ...
   const groupDb = client.db("cluster0").collection("groups");
   const stuff = await groupDb.find().sort({score:-1}).limit(5).toArray();


    res.json({ success: true, data: stuff });
  } catch (e) {
...
app.get('/incrementScore',async (req,res)=>{
  try{
    const client = await connectMongo();
    const userDb = client.db("cluster0").collection("users");
    const stuff = await userDb.findOne({emailAddress: "tharunsevvel@gmail.com"});
    const data = stuff.userGroups;
    for (const groupName of data)
    {
      await client.db("cluster0").collection("groups").updateOne({name:groupName},{$inc: {score:100}});
    }
    res.json({ success: true });
  }
  catch(e)
  {
    console.error('increment score error:', e);
    res.status(500).json({ success: false, error: e.message || 'internal error' });
  }
})