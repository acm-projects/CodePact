...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      const stuff = await fetch("http://localhost:3000/leaderboard")
      const variable = await stuff.json();
      console.log(variable.data);
      setLeaderboardData(variable.data); 
      console.log(leaderboardData[0]);
      console.log(leaderboardData[0]?.name);
    };
    fetchLeaderboard();
  },[])
        ...
        >
          ...
          <div className="space-y-4">
            <LeaderboardEntry
              ...
              points={leaderboardData[0]?.score}
              color="yellow-400"
              highlight
            />
            <LeaderboardEntry
              ...
              points={leaderboardData[1]?.score}
              color="gray-400"
            />
            <LeaderboardEntry
              rank={3}
              name={leaderboardData[2]?.name}
              points={leaderboardData[2]?.score}
              color="fuchsia-400"
            />
            <LeaderboardEntry
              rank={4}
              name={leaderboardData[3]?.name}
              points={leaderboardData[3]?.score}
              color="gray-400"
            />
            <LeaderboardEntry
              rank={5}
              name={leaderboardData[4]?.name}
              points={leaderboardData[4]?.score}
              color="gray-400"
            />
          </div>
        </div>
